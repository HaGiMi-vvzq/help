...
class AgentSmokeTests(unittest.TestCase):
    ...
    def _login(self, username: str = "alice", password: str = "123456") -> tuple[dict, dict]:
        ...
        if response.status_code == 401:
            ...
            if reg.status_code != 200:
                raise AssertionError(f"Failed to register test user: {reg.text}")
            response = self.client.post("/api/v1/auth/login", json={"username": username, "password": password})
    ...
    def _create_session(self, headers: dict, title: str) -> int:
        response = self.client.post("/api/v1/agent/sessions", json={"title": title}, headers=headers)
        self.assertEqual(response.status_code, 200, response.text)
    ...
    def test_settings_api_key_check_does_not_persist_key(self):
        ...
        after = self.client.get("/api/v1/settings", headers=headers)
    ...
    def test_agent_follow_up_publish_flow_works_without_remote_ai(self):
        ...
        with patch("app.services.agent_executor._run_matching_in_background", new=AsyncMock(return_value=None)), patch(
            ...
            publish_response = self.client.post(
                f"/api/v1/agent/sessions/{session_id}/confirm-publish",
                json={"draft": draft},
                headers=headers,
            )
    ...
    def test_agent_publish_confirmation_message_persists_need_after_follow_up(self):
        ...
        mine_response = self.client.get("/api/v1/needs/mine", headers=headers)
    ...
    def test_upload_flow_uses_fallbacks_and_returns_publishable_draft(self):
        ...
        tasks_response = self.client.get(f"/api/v1/agent/sessions/{session_id}/tasks", headers=headers)
    ...
    def test_confirm_publish_can_publish_only_selected_pending_draft(self):
        ...
        session_response = self.client.get(f"/api/v1/agent/sessions/{session_id}", headers=headers)
    ...
    def test_permission_and_exception_paths_are_guarded(self):
        ...
        unauthorized = self.client.get(f"/api/v1/agent/sessions/{session_id}", headers=bob_headers)
        ...
        tasks_response = self.client.get(f"/api/v1/agent/sessions/{session_id}/tasks", headers=alice_headers)
        self.assertEqual(tasks_response.status_code, 200, tasks_response.text)
        waiting_task = next(task for task in tasks_response.json() if task["status"] == "waiting_user")

        retry = self.client.post(
            f"/api/v1/agent/sessions/{session_id}/tasks/{waiting_task['id']}/retry",
            headers=alice_headers,
        )
    ...
    def test_agent_dialogue_routes_demo_questions_without_stiff_tool_list(self):
        ...
        with patch(
            ...
            discover_response = self.client.post(
                f"/api/v1/agent/sessions/{discover_session}/chat",
        ...
        publish_response = self.client.post(
            f"/api/v1/agent/sessions/{publish_session}/chat",
        ...
        with patch(
            ...
            upload_response = self.client.post(
                f"/api/v1/agent/sessions/{upload_session}/chat",
    ...
    def test_user_can_apply_to_need_and_owner_can_accept(self):
        ...
        detail_response = self.client.get(f"/api/v1/needs/{need_id}", headers=bob_headers)
        self.assertEqual(detail_response.status_code, 200, detail_response.text)
        self.assertTrue(detail_response.json()["can_apply"])

        apply_response = self.client.post(
            f"/api/v1/needs/{need_id}/apply",
            headers=bob_headers,
        ...
        applications_response = self.client.get(
            f"/api/v1/needs/{need_id}/applications",
            headers=alice_headers,
        )
        ...
        accept_response = self.client.post(
            f"/api/v1/needs/applications/{application_id}/accept",
            headers=alice_headers,
        ...
        mine_response = self.client.get("/api/v1/needs/applications/mine", headers=bob_headers)
        self.assertEqual(mine_response.status_code, 200, mine_response.text)
        ...
        conversation_response = self.client.get(f"/api/v1/messages/1?needId={need_id}", headers=bob_headers)
    ...
    def test_single_need_selection_finalizes_unselected_pending_applications(self):
        ...
        bob_mine = self.client.get("/api/v1/needs/applications/mine", headers=bob_headers)
        ...
        iris_mine = self.client.get("/api/v1/needs/applications/mine", headers=iris_headers)
    ...
    def test_selecting_recommended_candidate_notifies_them_to_continue_conversation(self):
        ...
        conversations_response = self.client.get("/api/v1/messages/conversations", headers=iris_headers)
        ...
        conversation_response = self.client.get(
            f"/api/v1/messages/{alice_payload['user']['id']}?needId={need_id}",
    ...
    def test_user_can_list_needs_where_they_were_selected(self):
        ...
        selected_response = self.client.get("/api/v1/needs/selected/mine", headers=iris_headers)
    ...
    def test_delete_need_cancels_background_matching(self):
        ...
        with patch("app.services.match_engine.cancel_matching", new=AsyncMock(return_value=True)) as cancel_mock:
            delete_response = self.client.delete(f"/api/v1/needs/{need_id}", headers=headers)
    ...
    def test_delete_need_with_application_and_messages_cleans_related_records(self):
        ...
        delete_response = self.client.delete(f"/api/v1/needs/{need_id}", headers=alice_headers)

        self.assertEqual(delete_response.status_code, 200, delete_response.text)
        detail_response = self.client.get(f"/api/v1/needs/{need_id}", headers=alice_headers)
    ...
    def test_agent_can_recommend_existing_needs_for_user(self):
        ...
        draft_response = self.client.post(
            "/api/v1/agent/draft-application-message",
            headers=bob_headers,
            json={
                "need_id": need_id,
